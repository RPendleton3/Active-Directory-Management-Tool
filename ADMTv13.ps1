...
Function Edit_Info{
	...
	{	
	Write-Host " "
	Write-Host "Enter Information to Replace Current Information for $Option"
	$Data = Read-Host
	$Data = IsDataThere -Name $Data
	$Data = $Data.substring(0,1).toupper()+$Data.substring(1)
	Write-Host " "
	Write-Host "Change $User $Option to $Data" 
	Write-Host "Please Note this will take effect Immediately!"
	$Valid = Read-Host -Prompt 'Y/N'
	$Valid = IsDataThere -Name $Valid
	switch($Valid)
	{
		'Y'
		{
			...
			switch($Option)
			{
				'FirstName'
				{
					Invoke-Command -ComputerName $ComputerName -ArgumentList $User, $Data -Credential ($Credential) -ScriptBlock { Set-ADUser $args[0] -GivenName $args[1] }
					break
				}
				'LastName'
				{
					Invoke-Command -ComputerName $ComputerName -ArgumentList $User, $Data -Credential ($Credential) -ScriptBlock { Set-ADUser $args[0] -Surname $args[1] }
					break
				}
				'AccountName'
				{					
					Invoke-Command -ComputerName $ComputerName -ArgumentList $User, $Data -Credential ($Credential) -ScriptBlock { Get-ADUser $args[0] | Set-ADUser -SamAccountName $args[1] }	
                    Invoke-Command -ComputerName $ComputerName -ArgumentList $User, $Data -Credential ($Credential) -ScriptBlock { Get-ADUser $args[1] | Rename-ADObject -NewName $args[1] }                    					
					break
		...
		Default
		{
			$temp = $true
			Write-Host " "
			Write-Host "An Error Occured, Please try again!"
			break
		}
	}
	}
...
while($Object -ne 'Exit'){
	...
	Write-Host "*EditUser*     Edit User Information"
    ...
    Switch ($Object)
    {
        ...
        'EditUser'
		{
			...
			{
				cls
				Write-Host "      ** Edit Domain User Information **"
				Write-Host "       *Type Exit at any time to quit*"
				Write-Host "*Type Return at any time to go back to menu*"
				Write-Host " "
				$Validation = Validate_User
                ...
                if ($Temp)
				{
					...
					{				           
			            Write-Host " "
		                Write-Host "     **Edit User Information**    "
		                Write-Host " "
		                Write-Host "*Show*     Show current list of Information able to change"
		                Write-Host "*Reset*    Reset current opperation"
		                Write-Host "*Return*   Return to main menu"
		                Write-Host "*Exit*     Exit the application"
			            Write-Host " "
			            $UserData = Read-Host -Prompt 'Enter Option'
						$UserData = IsDataThere -Name $UserData
						Switch ($UserData)
						{
							'Show'
							...
							'FirstName'
							{
								Edit_Info -Option "FirstName" -User $UserName
								break
							}
							'LastName'
							{
								Edit_Info -Option "LastName" -User $UserName
								break
							}
							'AccountName'
							{
								Edit_Info -Option "AccountName" -User $UserName
								break
							}
							Default
							{
